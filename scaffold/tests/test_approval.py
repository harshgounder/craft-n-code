...
def main():
    # use a fresh throwaway DB so persistence tests are deterministic
    with tempfile.TemporaryDirectory() as td:
        tmp_db = Path(td) / "signal.db"
        os.environ.setdefault("OLLAMA_API_KEY", "")  # force offline
        # seed a live db by running engine, then swap it in place
        env = dict(os.environ)
        proc = subprocess.run([sys.executable, str(ROOT / "engine" / "engine.py"), "--seed"],
                              capture_output=True, env=env, cwd=str(ROOT / "engine"))
        assert proc.returncode == 0, proc.stderr.decode()
        # point DB to temp copy by copying over existing signal.db path (engine main created it)
