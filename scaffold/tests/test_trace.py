...
def main():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        s = start_server(["--port", str(PORT), "--fixture", "happy", "--offline"], offline_env, errfile)
        wait_ready(s, errfile)
        try:
            stats = jget("/api/stats")
        ...
        s = start_server(["--port", str(PORT), "--fixture", "adversarial"], env, errfile)
        wait_ready(s, errfile)
        try:
            feed = jget("/api/feed")
        ...
        s = start_server(["--port", str(PORT), "--fixture", "ambiguous"], env, errfile)
        wait_ready(s, errfile)
        try:
            feed = jget("/api/feed")
            ids = [i["source_id"] for i in feed["items"]]
        ...
        s = start_server(["--port", str(PORT), "--fixture", "happy", "--offline"], key_env, errfile)
        wait_ready(s, errfile)
        try:
            stats = jget("/api/stats")
